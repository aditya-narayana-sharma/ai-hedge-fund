"""Managing Ollama models over HTTP, for when the server is not local.

Used when OLLAMA_BASE_URL points at another container (docker compose) or at
the host from inside one. The local path in ``ollama.py`` shells out to the
``ollama`` binary, which does not exist in that situation, so these helpers
drive the REST API instead and therefore take the URL as an argument.

Previously named ``docker.py``, which read like a Docker SDK wrapper and made
the signature difference look like an accidental divergence from
``ollama.py`` rather than a deliberate one.
"""
